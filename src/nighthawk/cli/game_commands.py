...
def start_game() -> None:
    ...
    if existing_saves:
        ...
        for slot, info in save_slots.items():
            if info:
                team_icon = "⚔️" if info["team"] == "red" else "🛡️"
                table.add_row(
                    str(slot),
                    info["username"],
...
def select_team_interactive() -> None:
    """⚔️ Select your team (Red Team or Blue Team)."""
    engine = get_game_engine()
...
def show_team_comparison() -> None:
    ...
    table = Table(title="⚔️ Team Comparison 🛡️", show_header=True, header_style="bold magenta")
    table.add_column("Aspect", style="yellow", justify="left")
    table.add_column("Red Team ⚔️", style="red", justify="left")
    table.add_column("Blue Team 🛡️", style="cyan", justify="left")
...
def show_detailed_comparison() -> None:
    ...
    red_panel = Panel(
        ...
        title="[red bold]⚔️ RED TEAM - Offensive Security[/red bold]",
        border_style="red",
    )
    ...
    blue_panel = Panel(
        ...
        title="[cyan bold]🛡️ BLUE TEAM - Defensive Security[/cyan bold]",
        border_style="cyan",
    )
...
def show_game_dashboard() -> None:
    ...
    stats_table.add_row(
        ...
        "⏱️ Playtime:", f"{profile.stats.playtime_minutes} min",
    )
...
def show_stats() -> None:
    ...
    # Combat Stats
    combat_table = Table(title="⚔️ Combat Statistics", show_header=False)
